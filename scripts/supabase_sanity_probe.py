...
try:
    ...
    print("Ensure this script is run from the project root or the PYTHONPATH is set correctly.")
    sys.exit(1)
...
def run_supabase_probes():
    ...
    if not settings.SUPABASE_URL or not settings.SUPABASE_SERVICE_KEY:
        ...
        PROBE_RESULTS["batch_predictions_limit1"]["details"] = "Supabase URL/Key not set"
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            PROBE_RESULTS["predictions_count"]["status"] = f"Error ({response.status_code})"
    ...
    try:
        url_batch_preds = f"{settings.SUPABASE_URL}/rest/v1/batch_predictions?select=id&limit=1"
        response = requests.get(url_batch_preds, headers=headers, timeout=10)
        if response.status_code == 200:
            PROBE_RESULTS["batch_predictions_limit1"]["status"] = "OK"
            PROBE_RESULTS["batch_predictions_limit1"]["details"] = f"{len(response.json())} record(s) returned (expected 0 or 1)"
        else:
            PROBE_RESULTS["batch_predictions_limit1"]["status"] = f"Error ({response.status_code})"
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            bucket_found = any(b["name"] == settings.STORAGE_BUCKET_NAME for b in buckets)
            if bucket_found:
                PROBE_RESULTS["storage_bucket_check"]["status"] = "OK"
                PROBE_RESULTS["storage_bucket_check"]["details"] = f"Bucket '{settings.STORAGE_BUCKET_NAME}' found."
            else:
                PROBE_RESULTS["storage_bucket_check"]["status"] = "Not Found"
                PROBE_RESULTS["storage_bucket_check"]["details"] = f"Bucket '{settings.STORAGE_BUCKET_NAME}' not found."
        else:
            PROBE_RESULTS["storage_bucket_check"]["status"] = f"Error ({response.status_code})"
...
if __name__ == "__main__":
    ...
    for probe, result in PROBE_RESULTS.items():
        print(f"{probe.replace('_', ' ').title()}: [{result['status']}] - {result['details']}")
    print("-------------------------------------")
