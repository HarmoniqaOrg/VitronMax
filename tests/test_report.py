...
def test_report_endpoint_valid_smiles() -> None:
    ...
    with patch("app.report.BBBPredictor") as MockPredictor:
        ...
        assert (
            "attachment; filename=vitronmax_report"
...
def test_report_endpoint_invalid_smiles() -> None:
    ...
    with patch("app.report.BBBPredictor") as MockPredictor:
        ...
        assert (
            "invalid" in str(response_json).lower()
            or "error" in str(response_json).lower()
        )
...
def test_report_endpoint_empty_smiles() -> None:
    ...
    assert (
        "error" in str(error_response).lower() or "value" in str(error_response).lower()
    )
