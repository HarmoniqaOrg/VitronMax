...
class TestSupabaseClientInit:
    ...
    def test_init_configured(
        self, mock_settings_configured: None, caplog: pytest.LogCaptureFixture
    ) -> None:
    ...
    def test_init_unconfigured(
        self, mock_settings_unconfigured: None, caplog: pytest.LogCaptureFixture
    ) -> None:
...
class TestSupabaseClientPatchJson:
    ...
    async def test_patch_json_success(
        ...
        mock_response.status_code = (
            204  # Common success for PATCH with Prefer: return=minimal
        )
...
class TestSupabaseClientStoreBatchPredictionItem:
    ...
    async def test_store_batch_prediction_item_payload_construction_error_message(
        ...
        assert (
            supabase_client._post_json.call_args[0][1]["error_message"] == error_message
        )
...
class TestSupabaseClientCreateBatchJob:
    ...
    async def test_create_batch_job_not_configured(
        self,
        supabase_client: SupabaseClient,
        mock_settings_unconfigured: None,
        caplog: pytest.LogCaptureFixture,
    ) -> None:
        """Test create_batch_job when client is not configured."""
        client = (
            SupabaseClient()
        )  # This instance will pick up the unconfigured settings
        assert not client.is_configured
        ...
        with caplog.at_level("WARNING"):
            ...
            result = await client.create_batch_job(
                job_id=job_id, filename="test_filename.csv", total_molecules=100
            )
        assert result is None
...
class TestSupabaseClientStorePrediction:
    ...
    async def test_store_prediction_success(
        ...
        result = await client.store_prediction(
            smiles=smiles, probability=probability, model_version=model_version
        )
    ...
    async def test_store_prediction_api_error(
        ...
        with caplog.at_level("ERROR"):
            result = await client.store_prediction(
                smiles=smiles, probability=probability
            )
...
class TestSupabaseClientUpdateBatchJobStatus:
    ...
    @patch(
        "app.db.httpx.AsyncClient"
    )  # Patch where AsyncClient is imported/used by SupabaseClient
    async def test_update_batch_job_status_success(
        ...
        type(mock_response).content = PropertyMock(
            return_value=json.dumps([expected_response_data]).encode("utf-8")
        )

        mock_cli_instance.patch.return_value = mock_response
        mock_async_client_constructor.return_value.__aenter__.return_value = (
            mock_cli_instance  # Configure for 'async with'
        )

        result = await client.update_batch_job_status(job_id=job_id, status="COMPLETED")
    ...
    async def test_update_batch_job_status_api_error(
        ...
        call_args, call_kwargs = mock_cli_instance.patch.call_args
        assert (
            call_args[0] == f"{client.url}/rest/v1/batch_predictions?id=eq.{job_id}"
        )  # Corrected path
    ...
    async def test_update_batch_job_status_not_configured(
        ...
        client._patch_json.assert_called_once_with(
            f"/rest/v1/batch_predictions?id=eq.{job_id}",
            {"status": "PENDING"},  # Corrected path & dynamic job_id
        )
